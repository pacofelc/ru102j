...
public class SiteStatsDaoRedisImpl implements SiteStatsDao {
    ...
    private void updateOptimized(Jedis jedis, String key, MeterReading reading) {
        ...
        Response<Long> ret1 =tx.hset(key, SiteStats.reportingTimeField, reportingTime);
        Response<Long> ret2 =tx.hincrBy(key, SiteStats.countField, 1);
        Response<Long> ret3 =tx.expire(key, weekSeconds);

        this.compareAndUpdateScript.updateIfGreater
                (tx, key, SiteStats.maxWhField, reading.getWhGenerated());
        this.compareAndUpdateScript.updateIfLess
                (tx, key, SiteStats.minWhField, reading.getWhGenerated());
        this.compareAndUpdateScript.updateIfGreater
                (tx, key, SiteStats.maxCapacityField, getCurrentCapacity(reading));
